...
parallel_simulations <- sfLapply(1:nrow(run_in_parallel), function(k) {
  ...
  for (sim in 1:chunk_size) {
    ...
    entropy <- label_parallel_runs_beta(matrix = entropy, beta = beta, simulation = sim, chunk = chunk)
    # Calculate total task distribution
    totalTaskDist <- X_tot / gens
    totalTaskDist <- label_parallel_runs_beta(matrix = totalTaskDist, beta = beta, simulation = sim, chunk = chunk)
    # Create thresh table
    threshMat <- label_parallel_runs_beta(matrix = threshMat, beta = beta, simulation = sim, chunk = chunk)
