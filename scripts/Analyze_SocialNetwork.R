...
files <- list.files("output/Rdata/_ProcessedData/Graphs/Sigma0-Epsilon0.01-Beta1.1/", full.names = TRUE)
soc_networks <- list()
...
files <- list.files("output/Rdata/_ProcessedData/Thresh/Sigma0-Epsilon0.01-Beta1.1/", full.names = TRUE)
thresh_data <- list()
for (i in 1:length(files)) {
  load(files[i])
